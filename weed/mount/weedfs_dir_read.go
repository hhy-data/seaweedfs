...
func (wfs *WFS) doReadDirectory(input *fuse.ReadIn, out *fuse.DirEntryList, isPlusMode bool) fuse.Status {
	...
	if input.Offset >= dh.entryStreamOffset {
		...
		if len(dh.entryStream) == 0 && input.Offset > dh.entryStreamOffset {
			...
			loadErr := wfs.metaCache.ListDirectoryEntries(context.Background(), dirPath, "", false, skipCount+int64(batchSize), func(entry *filer.Entry) bool {
		...
		if !isEarlyTerminated {
			...
			loadedCount := 0
			bufferFull := false
			loadErr := wfs.metaCache.ListDirectoryEntries(context.Background(), dirPath, lastEntryName, false, int64(batchSize), func(entry *filer.Entry) bool {
