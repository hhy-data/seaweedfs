...
type FilerOptions struct {
	...
	concurrentChunkUploadLimit *int
	debug                      *bool
	debugPort                  *int
	localSocket                *string
...
func init() {
	...
	f.concurrentChunkUploadLimit = cmdFiler.Flag.Int("concurrentChunkUploadLimit", 64, "limit total concurrent chunk uploads to prevent memory bloat")
...
func (fo *FilerOptions) startFiler() {
	...
	fs, nfs_err := weed_server.NewFilerServer(defaultMux, publicVolumeMux, &weed_server.FilerOption{
		...
		ConcurrentChunkUploadLimit: int64(*fo.concurrentChunkUploadLimit),
