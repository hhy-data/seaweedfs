...
func fetchChunkRange(ctx context.Context, buffer []byte, lookupFileIdFn wdclient.LookupFileIdFunctionType, fileId string, cipherKey []byte, isGzipped bool, offset int64, invalidator CacheInvalidator) (int, error) {
	urlStrings, err := lookupFileIdFn(ctx, fileId)
	if err != nil {
		glog.ErrorfCtx(ctx, "operation LookupFileId %s failed, err: %v", fileId, err)
		return 0, err
	}
	n, err := util_http.RetriedFetchChunkData(ctx, buffer, urlStrings, cipherKey, isGzipped, false, offset, fileId)
	if err != nil {
		// Self-heal: drop stale volume locations and retry once with fresh ones
		err = retryFetchWithFreshLocations(ctx, invalidator, lookupFileIdFn, fileId, urlStrings, err, func(newUrls []string) error {
			var err2 error
			n, err2 = util_http.RetriedFetchChunkData(ctx, buffer, newUrls, cipherKey, isGzipped, false, offset, fileId)
			return err2
		})
	}
	return n, err
